...
export const ablunitRun = async (options: TestRun, res: ABLResults, cancellation: CancellationToken) => {
	...
	const parseRuntimeError = (stdout: string): string | false => {
		// TODO - parse any runtime errors, such as...
		// "Unable to build type info for propathTest.p: Invalid datatype specified: OpenEdge.Web.WebResponse. Specify a datatype such as 'character' or the name of a class. (5638)""
		const promsgRegex = /^.* \(\d+\)/
