import { commands, tests, window, workspace,
	...
	TextDocument, Uri, WorkspaceFolder, FileType, CancellationError } from 'vscode'
...
export async function activate (context: ExtensionContext) {
	...
	const startTestRun = (request: TestRunRequest, cancellation: CancellationToken) => {
		...
		// cancellation.onCancellationRequested(() => {
		// 	log.error("cancellation requested", run)
		// 	run.end()
		// })
...
function getWorkspaceTestPatterns () {
	...
	log.info("retVal.length=" + includePatterns.length + "," + excludePatterns.length)
...
async function refreshTestTree (controller: TestController, token: CancellationToken) {
	...
	const checkCancellationToken = (message: string) => {
		if (token.isCancellationRequested || abort) {
			log.warn('cancellation requested ' + elapsedTime())
			log.trace('token.isCancellationRequested=' + token.isCancellationRequested + ", abort=" + abort + ", message=" + message)
			logResults()
			throw new CancellationError()
		}
	}
	const { includePatterns, excludePatterns } = getWorkspaceTestPatterns()
	// const testfiles: { file: TestItem | undefined, data: ABLTestData | undefined }[] = []


	log.info(' --- removeExcludedFiles ' + elapsedTime())
	removeExcludedFiles(controller, excludePatterns, token)

	log.info(" --- starting includePattern loop")
	let incCount = 0
	for (const includePattern of includePatterns) {
		incCount++
		log.info(" --- includePattern.pattern-1=" + includePattern.pattern + " " + includePattern.baseUri)
		const prom = workspace.findFiles(includePattern, undefined, undefined, token).then((foundFiles) => {
			log.info(" ---- foundFiles.length = " + foundFiles.length + ' ' + elapsedTime())
			foundFiles =  foundFiles.filter(uri => !isFileExcluded(uri, excludePatterns))
			log.info(" ---- foundFiles.filtered.length = " + foundFiles.length + ' ' + elapsedTime())
			return foundFiles
		}, (err) => {
			log.error('caught error searching included files for tests: ' + err)
			throw err
		})
		log.info(" --- prom returned")
		const patternFiles = await prom
		log.info(" --- patternFiles.length = " + patternFiles.length)
		filelist.push(...patternFiles)
		log.info(" --- filelist.info = " + filelist.length)
		checkCancellationToken('gather included files - searched ' + incCount + '/' + includePatterns.length + ' patterns')
	}
	log.info(" --- filelist.length = " + filelist.length + ' ' + elapsedTime())

	for (const file of filelist) {
		searchCount++
		// checkCancellationToken("gather included files - " + includePattern.pattern + ", " + wsFile.fsPath)
		checkCancellationToken('update tests from disk - parsed' + searchCount + '/' + filelist.length + ' files ')
...
async function createDir (uri: Uri) {
	...
	return workspace.fs.stat(uri).then((stat) => {
		if (!stat) {
			...
			log.info('create-1: ' + uri.fsPath)
			return workspace.fs.createDirectory(uri)
		}
	}, () => {
		log.info('create dir (' + uri.fsPath + ')')
		return workspace.fs.createDirectory(uri)
	})
}
