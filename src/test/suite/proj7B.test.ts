...
import { sleep, waitForExtensionActive } from '../testCommon'
import { getTestController } from '../../extension'
...
function getChildTestCount (type: string | undefined, items: TestItemCollection) {
	...
	// 	if (type === 'ABLTestFile' && item instanceof ABLTestFile) {
	// 		count++
	// 	}
	// 	switch (type) {
	// 		case 'ABLTestFile':
	// 			if (item instanceof ABLTestFile) { count++ }
	// 			break
	// 		case undefined:
	// 			count++
	// 			break
	...
	// 	}
	// 	log.info('count=' + count + ', item: ' + item.uri)
	// 	count += getChildTestCount(type, item.children)
	// }
	// return count
}
